...
// This Toll system bye SA-MP Edited By LoliHunter/Magallan 
// Show The Credit From SA-MP and LoliHunter 
// Send the report to my discord DEV MGRP#6109 If the Other Dev delete this credit
// Credit LoliHunter Copyright 2021
// Credit SA-MP Copyright 2021

// Main configuration
#define TollFS (1) 						// 0 = Include, 1 = Filterscript
#define TollCost (200) 					// How much it costs to pass the tolls
...
#define TollOpenDistance (7.5) 			// The distance in units the player can be from the icon to open the toll
...
#define DiLockKontol ("[SERVER] Gerbang Ditutup Oleh SAPD")
#define GakAdaDuid ("[ERROR] Anda Tidak Mempunyai Uang Untuk Membuka Gerbang Toll.")
#define Dadah ("[SERVER] Terimakasih Hati Hati Di Jalan.")
#define BuruanAsu ("[SERVER] Anda Mempunyai Waktu 6 Detik Untuk Lewat,Pastikan Anda Tidak Terjepit.")
...
new L_a_RequestAllowedCop, // The same timer for all /toll changes
	...
	public OnPlayerCommandText(playerid, cmdtext[])//if(IsKeyJustDown(KEY_CROUCH, newkeys, oldkeys))
	{
	    if(!strcmp(cmdtext, "/toll", true, 9))
	    {
...
public Toll_OnPlayerPickUpPickup(playerid, pickupid) // Needs forwarding in the OnPlayerPickup function of the script
{
    if(pickupid == L_a_Pickup[0] || pickupid == L_a_Pickup[1] || pickupid == L_a_Pickup[2] || pickupid == L_a_Pickup[3] || pickupid == L_a_Pickup[4] || pickupid == L_a_Pickup[5])
	{
		if(PDDuty[playerid])
		{
			ProxDetector(20.0, playerid, "Toll Guard says: Halo Kak, Anda Ingin Lewat?", COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
			SendClientMessage(playerid, COLOR_GOLD, "[INFO] Gunakan CMD /paytoll Untuk Membayar Toll.");
			return 1;
		}
		new szCostString[56];
		format(szCostString, 56, "Toll Guard says: Hallo, Total Harga %d Dollars.", TollCost);
	...
	if(L_a_TollPerson[playerid][0] != INVALID_TOLL_ID)
	{
		...
		else
		{
		    switch(L_a_TollPerson[playerid][0])
		    {
		        case RichmanToll:
		        {
				    if(IsPlayerInRangeOfPoint(playerid, 0.9, 619.2152, -1174.6862, 20.5124) || IsPlayerInRangeOfPoint(playerid, 0.9, 612.2329, -1202.9524, 18.1094))
				    {
		                Toll_CloseToll(L_a_TollPerson[playerid][0]);
	        			L_a_TollPerson[playerid][0] = 0;
					}
				...
				case FlintToll:
				{
				    if(IsPlayerInRangeOfPoint(playerid, 0.9, 29.2651,-1521.5536,4.8644) || IsPlayerInRangeOfPoint(playerid, 0.9, 73.2545, -1541.4111, 5.2093))
				    {
		                Toll_CloseToll(L_a_TollPerson[playerid][0]);
	        			L_a_TollPerson[playerid][0] = 0;
					}
				...
				case LVToll:
				{
				    if(IsPlayerInRangeOfPoint(playerid, 0.9, 1797.5039, 714.3255, 14.4545) || IsPlayerInRangeOfPoint(playerid, 0.9, 1775.9889, 691.7012, 15.9699))
				    {
		                Toll_CloseToll(L_a_TollPerson[playerid][0]);
	        			L_a_TollPerson[playerid][0] = 0;
					}
...
{ // Thanks to Nexus for mapping!
	...
	printf("Pay Toll By SA-MP");
	printf("____________________________________");
	printf("              TTD  (c) SA-MP 2020    ");
...
Toll_TimePassedCivil(TollID, playerid) // People have to wait <TollDelayCivilian> seconds between every /paytoll on the same toll
{
	new L_i_tick = GetUnixTime();
	if(L_a_Pickup[TollID*2])
	{
	    SendClientMessage(playerid, COLOR_RED, "Gerbang Sudah Terbuka, Kamu Harus Menunggu Tertutup Untuk Membayar (( Jika Melanggar/Menerobos PG )).");
	    return 0;
	}
	if(aTolls[TollID][E_tAllowReq] > L_i_tick && aTolls[TollID][E_tAllowReq] != 0)
	{
	    new TollString[64];
		format(TollString, 64, "[ERROR] Gerbang Sudah Terbuka (( Jika Menerobos Tol PG ))", TollDelayCivilian);
...
Toll_TimePassedCops(playerid) // Cops have to wait for <TollDelayCop> seconds between every /toll (Global)
{
	...
	{
	    new TollString[63];
		format(TollString, 63, "[ERROR] Gerbang Sudah Terbuka Oleh Kamu!.", TollDelayCop);
		SendClientMessage(playerid, COLOR_RED, TollString);
		return 0;
	}
...
public ycmd_opentoll(playerid, cmdtext[])
{
		...
		{
		    SendClientMessage(playerid, COLOR_RED, "[ERROR] Anda Harus Di Dekat Gerbang Toll!");
			return 1;
		}
	#endif
    ...
    {
	    //SendClientMessage(playerid, COLOR_RED, "[SERVER] Anda Tidak Dekat Dengan Gerbang Toll");
	    return 1;
    }
    ...
    if(!PDDuty[playerid])
	{
	    ...
	    {
	        ProxDetector(20.0, playerid, DiLockKontol, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
			return 1;
		}
		...
		if(IsPlayerInAnyVehicle(playerid))
		{
		    SendClientMessage(playerid, COLOR_RED, GakAdaDuid);
		    return 1;
		}
		format(L_sz_MessageString, 156, "** %s Membayar Sebesar 50$ Ke Penjaga Toll.", GetICName(playerid), TollCost);
		GivePlayerMoney(playerid, -TollCost);
		ProxDetector(10.0, playerid, L_sz_MessageString, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
		GameTextForPlayer(playerid, "~r~Selamat Jalan", 1000, 1);
		#if TollFS == 0
	        PlayerInfo[playerid][pCash] -= TollCost;
			#if TollPayCops > 0
		        FactionInfo[1][fFunds] += (TollCost/10)*TollPayCops;
		        FactionInfo[6][fFunds] += (TollCost/10)*TollPayCops;
	        #endif
        #endif
	}

	ProxDetector(20.0, playerid, Dadah, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
	SendClientMessage(playerid, COLOR_RED, BuruanAsu);
...
public ycmd_toll(playerid, cmdtext[])
{
	...
	else if(strcmp(L_sz_Input, "flint", true, strlen(L_sz_Input)) == 0)
	{
	    if(aTolls[FlintToll][E_tLocked] == 0)
	    {
	        ...
	        Toll_CloseToll(FlintToll);
		    format(L_sz_MessageString, 256, "", L_sz_Rank, L_sz_CopName, L_sz_Faction);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
   	if(newkeys & KEY_CROUCH)
	{
		...
		{
            OnPlayerCommandText(playerid, "/paytoll");
        }
	}
	return 1;
}
